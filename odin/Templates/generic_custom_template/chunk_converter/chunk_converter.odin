...
convert_collision_chunk :: proc(input_path, output_path: string, direction: Command) -> bool {
	...
	case .JSON_TO_BINARY:
		...
		if !load_ok {
			fmt.printf(
				"Failed to load chunk!: convert_collision_chunk :: proc (%s, %s, %s)",
				input_path,
				output_path,
				direction,
			)
...
find_chunks_in_dir :: proc(dir_path: string, files: ^[dynamic]FileInfo, recursive: bool) {
	...
	for info in file_infos {
		...
		if strings.contains(info.name, "chunk_") {
			...
			if chunk_type != "" &&
			   (strings.has_suffix(info.name, ".json") || strings.has_suffix(info.name, ".dat")) {
				append(
					files,
...
load_visual_chunk_binary :: proc(filepath: string) -> (Visual_Chunk, bool) {
	...
	if len(data) < 8 + CHUNK_SIZE * CHUNK_SIZE * 4 {
	...
	fmt.printf("Load_Visual_Chunk_Binary\n")
	for i in 0 ..< entity_count {
		entity := Entity{}
		entity.kind = cast(EntityKind)(cast(^u32)&data[offset])^;offset += 4
		fmt.printf("EntityKind: %v\n", entity.kind)
...
save_visual_chunk_binary :: proc(filepath: string, chunk: Visual_Chunk) -> bool {
	...
	entity_size := len(chunk.entities) * (2 * 4 + 4) // 2 floats + 1 u32 per entity
	...
	for entity in chunk.entities {
		(cast(^u32)&data[offset])^ = cast(u32)entity.kind;offset += 4
