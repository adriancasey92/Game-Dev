...
//Resets the player to default values at position pos
resetPlayer :: proc(pos: Vec2) {
...
//Handles all the player specific updates
update_player :: proc(dt: f32) {
	//get player entity
	p := get_player()

	//For keeping our input value so we continue to move outwards when jumping from the edge of a platform
	//if player isn't side jumping make input 0
	if !p.side_jump {
		p.input = {}
	} else {
		//if we are side jumping, increment the timer
		p.side_jump_timer += dt
		//if we have been side jumping for more than 1 second
		//p.side_jump = false so that we can then move normally again
...
draw_player :: proc(fade: f32) {
	fmt.printf("draw_player - replaced by draw_generic_entity\n")
	/*p := get_player()
	...
	origin := Vec2 {
		...
		anim_texture.document_size.y + 2, // -1 because there's an outline in the player anim that takes an extra pixel
	}
