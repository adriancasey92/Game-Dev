...
Player :: struct {
	...
	anims:  [Animation_Name]Animation,
	anim:   Animation,
	flip_x: bool,
}
...
//handles user input
handle_input :: proc() -> Vec2 {
	input: Vec2
	if rl.IsKeyDown(.LEFT) || rl.IsKeyDown(.A) {
		input.x -= 1
		player.dir = .Left
		if player.anim.atlas_anim != .Frog_Move {
			fmt.printf("SETTING ANIMATION TO FROG_MOVE LEFT\n")
			player.anim = player.anims[.Frog_Move]
		}
	}
	if rl.IsKeyDown(.RIGHT) || rl.IsKeyDown(.D) {
		...
		player.dir = .Right
		if player.anim.atlas_anim != .Frog_Move {
			fmt.printf("SETTING ANIMATION TO FROG_MOVE RIGHT\n")
			player.anim = player.anims[.Frog_Move]
		}
	}
	...
	if rl.IsKeyDown(.SPACE) {
		input.y -= 1
		player.dir = .Up
		if player.anim.atlas_anim != .Frog_Jump {
			player.anim = player.anims[.Frog_Jump]
		}
	}
...
update :: proc() {
	input: Vec2
	fmt.printf("Input %i,%i: \n", input.x, input.y)
	fmt.printf("Curr Anim: %s\n", player.anim.atlas_anim)

	//seperate user input from update
	input = handle_input()

	//update animation
	animation_update(&player.anim, rl.GetFrameTime())

	//flip animation by direction
	...
	//check if player has moved, if not set Frog_Idle
	old_pos := player.pos
	if input.y != 1 {
		input.y += 500 * rl.GetFrameTime()
	}
	...
	if old_pos == player.pos {
		...
		//if !climbing
		//if ! swinging etc
		if player.anim.atlas_anim != .Frog_Idle {
			player.anim = player.anims[.Frog_Idle]
		}
	}
}
...
init_player :: proc() {
	...
	for n, idx in Animation_Name {
		if idx != 0 {
			fmt.printf("Creating animation : %s\n", n)
			player.anims[n] = animation_create(n)
		} else {
			fmt.printf("Anim NIL\n")
		}
	}
