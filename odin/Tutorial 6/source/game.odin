	...
	if g_mem.in_menu {
		...
		for r, i in rects {
			if rl.CheckCollisionPointRec(
				rl.GetScreenToWorld2D(rl.GetMousePosition(), ui_camera()),
				r,
			) {
...
draw :: proc() {
	...
	if g_mem.editing {
		...
		rl.DrawTextEx(
			font,
			fmt.ctprintf("%v", g_mem.long_cat_spawns),
			{10, PIXEL_WINDOW_HEIGHT - 30},
			20,
			0,
			rl.WHITE,
		)
...
init :: proc() {
	...
	bg_shader_str := strings.string_from_ptr(
		raw_data(BACKGROUND_SHADER_DATA),
		len(BACKGROUND_SHADER_DATA),
	)
	ground_shader_str := strings.string_from_ptr(
		raw_data(GROUND_SHADER_DATA),
		len(GROUND_SHADER_DATA),
	)
	ground_shader_vs_str := strings.string_from_ptr(
		raw_data(GROUND_SHADER_VS_DATA),
		len(GROUND_SHADER_VS_DATA),
	)
	...
	g_mem^ = Game_Memory {
		...
		ground_shader     = rl.LoadShaderFromMemory(
			temp_cstring(ground_shader_vs_str),
			temp_cstring(ground_shader_str),
		),
